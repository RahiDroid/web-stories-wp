...
class Stories_Controller extends Stories_Base_Controller {
	...
	protected function get_attached_post_ids( array $posts ): array {
		...
		$publisher_logo_ids = array_filter( array_map( function ( $post ) {
			return (int) get_post_meta( $post->ID, Story_Post_Type::PUBLISHER_LOGO_META_KEY, true );
		}, $posts ) );

		return array_unique( array_merge( $thumb_ids, $publisher_logo_ids ) );
	}
